"""
fit_scoreline_shape.py — Fit v9's NB+Frank score shape parameters (r, kappa, theta_draw)
======================================================================================
...
all default values — the NB+Frank engine is connected, but the **dependence kappa and diagonal inflation theta were never fitted on data**
(kappa=0 equals degenerating to independent NB, theta=0 equals no draw inflation). The engine is idling.

This script, given DC's lambda_h, lambda_a (without touching attack/defense/Elo prior), fits
(r, kappa, theta_draw) these 3 global scalars on earlier data, then evaluates their impact on
**derived markets** (exact score / over-under 2.5 / BTTS / 1X2) on a later held-out set, and finally writes
the fitted shape_ back to the model and saves a new version.

Key: only fits 3 global scalars; lambda still comes from DC (with Elo prior). The fit/eval time split isolates
shape's contribution — even if DC has seen this data, the "fitted shape vs default shape" comparison is still
clean because both use the same set of DC lambdas, only the shape differs.
...
def _evaluate(fsm, eval_df, shape: dict) -> dict:
    """Evaluate metrics (lower is better, except acc) with given shape on eval set."""
    K = fsm.max_goals
...
def fit_shape(dc, fit_df: pd.DataFrame, max_fit_rows: int = 8000, recency_damping: float = 0.0):
    """Fit (r, kappa, theta_draw) of dc._scoreline_model on fit_df. Modifies shape_ in place."""
    ...
    if max_fit_rows and len(df) > max_fit_rows:
        df = df.sort_values("date").tail(max_fit_rows)   # Global scalars, latest N matches are sufficient and faster
...
def load_matches_sql() -> pd.DataFrame:
    """Pull all matches from SQL for shape fitting."""
...
def main():
    ...
    ap.add_argument("--sql", action="store_true", help="Pull data from SQL database (alternative to --data)")
    ...
    if a.sql:
        ...
        ap.error("Please provide --data path or --sql")
